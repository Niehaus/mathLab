...
public class Manager : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int i = 1; i < _qntCenouras + 1; i++) {
            StartCoroutine(Spawn(i * _cenoura.spawn, _cenoura.transform, _rand.NextDouble()));
        }
        for (int i = 1; i < _qntCoins + 1; i++) {
            StartCoroutine(Spawn(i * _coins1.spawn, _coins1.transform, _rand.NextDouble()));
        }
        for (int i = 1; i < _qntHearts + 1; i++) {
            StartCoroutine(Spawn(i * _pao.spawn, _pao.transform, _rand.NextDouble()));
        }
    }
